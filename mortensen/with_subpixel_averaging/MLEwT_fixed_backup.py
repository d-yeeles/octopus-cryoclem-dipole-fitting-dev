...
#            'ftol': 1e-1,
        ...
        if not conditionA:
            ...
            scores.append(second_attempt_score)


            phi_est_new = np.arctan2(second_attempt_params[1], second_attempt_params[0]) % (2*np.pi)
            conditionA = abs(phi_est_new) >= np.pi
            if not conditionA:
                print('    Attempt 3')
                rand_ang = np.random.uniform(0, 2 * np.pi)
                third_attempt_pinit = first_attempt_params
                third_attempt_pinit[0] = np.sin(theta_est)*np.cos(rand_ang)
                third_attempt_pinit[1] = np.sin(theta_est)*np.sin(rand_ang)
                third_attempt_pinit[2] = np.cos(theta_est)
                ...
                scores.append(third_attempt_score)


                phi_est_new = np.arctan2(third_attempt_params[1], third_attempt_params[0]) % (2*np.pi)
                conditionA = abs(phi_est_new) >= np.pi
                if not conditionA:
                    print('    Attempt 4')
                    rand_ang = np.random.uniform(0, 2 * np.pi)
                    fourth_attempt_pinit = first_attempt_params
                    fourth_attempt_pinit[0] = np.sin(theta_est)*np.cos(rand_ang)
                    fourth_attempt_pinit[1] = np.sin(theta_est)*np.sin(rand_ang)
                    fourth_attempt_pinit[2] = np.cos(theta_est)
                    ...
                    scores.append(fourth_attempt_score)
