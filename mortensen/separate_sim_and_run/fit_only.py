...
#    psf_image_output = tifffile.imread(image_path)
    psf_image_output = np.loadtxt(image_path, delimiter=",")
...
if __name__ == "__main__":
    ...
    runs = range(1)
    thetas = np.arange(22.5*np.pi/180, np.pi/2 + 0.001, 22.5*np.pi/180)
    phis = np.arange(0*np.pi/180, 2*np.pi + 0.001, 45*np.pi/180)
    ...
    for run in runs:
      for theta in thetas:
        for phi in phis:
          ...
          # Define output paths
          image_output_path = f'./sims/sim_theta{ceil(theta_deg):03}_phi{ceil(phi_deg):03}_run{int(run+1)}.csv'
          params_output_path = f"./sims/params_theta{ceil(theta_deg):03}_phi{ceil(phi_deg):03}_run{int(run+1)}.m"
...
#          file.close()
          ...
          results_path = "./fitting_results_mortensen_on_itself.py"
          ...
          # run the fit
          results, ground_truth = run_mortensen_fit(psf_image, image_output_path, params_output_path)
