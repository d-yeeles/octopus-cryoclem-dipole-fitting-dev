...
#            costFunction = @(x) -lnpdf(image, x)n

        eng.eval('nonlcon = @(x) deal([], x(4)^2 + x(5)^2 + x(6)^2 - 1)', nargout=0)
)

        # Define the initial guess
        stratValues = matlab.double([pinit[0], pinit[1], pinit[2], pinit[3], pinit[4], pinit[5]])
        ...
        solution = eng.fmincon(eng.workspace['objective'], startValues, blank, blank, blank, blank, lb, ub, eng.workspace['nonlcon'])
        # Print the results
