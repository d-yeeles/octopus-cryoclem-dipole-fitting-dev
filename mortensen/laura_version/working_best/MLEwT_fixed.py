...
#        print(f"    initial phi = {round(np.arctan2(pinit[1], pinit[0])*180/np.pi % 360)}")
        ...
        bounds = [(-1, 1), (-1, 1), (0, 1), (-51*18/2, 51*18/2), (-51*18/2, 51*18/2), (1e-6, None)]
...
#        res = minimize(ll.Value, pinit, method='Powell', bounds=bounds)#, constraints=constraints)
#        est=res.x    #[0]
#        # warnflag=res[5]
#
#        first_attempt_params = est
...
#
#        condition1 = abs(theta_est - np.pi/2) <= 1*np.pi/180
#        condition2 = any(abs(phi_est - angle) <= 10*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
#
#        if condition1 or condition2:
#
#          print('trying fudge')
#
#          secondStartValues = pinit
#          secondStartValues[0] = np.sin(theta_est)*np.cos(phi_est - np.pi)
#          secondStartValues[1] = np.sin(theta_est)*np.sin(phi_est - np.pi)
#          secondStartValues[2] = np.cos(theta_est)
#          res = minimize(ll.Value, secondStartValues, method='Powell', bounds=bounds)
#          second_attempt_params = res.x    #[0]
#          second_attempt_score = ll.Value(second_attempt_params)
#
#          thirdStartValues = pinit
#          thirdStartValues[0] = np.sin(theta_est - np.pi/2)*np.cos(phi_est)
#          thirdStartValues[1] = np.sin(theta_est - np.pi/2)*np.sin(phi_est)
#          thirdStartValues[2] = np.cos(theta_est - np.pi/2)
#          res = minimize(ll.Value, thirdStartValues, method='Powell', bounds=bounds)
#          third_attempt_params = res.x    #[0]
#          third_attempt_score = ll.Value(third_attempt_params)
#
#          fourthStartValues = pinit
#          fourthStartValues[0] = np.sin(theta_est - np.pi/2)*np.cos(phi_est - np.pi)
#          fourthStartValues[1] = np.sin(theta_est - np.pi/2)*np.sin(phi_est - np.pi)
#          fourthStartValues[2] = np.cos(theta_est - np.pi/2)
#          res = minimize(ll.Value, fourthStartValues, method='Powell', bounds=bounds)
#          fourth_attempt_params = res.x    #[0]
#          fourth_attempt_score = ll.Value(fourth_attempt_params)
#
#          params = [first_attempt_params, second_attempt_params, third_attempt_params, fourth_attempt_params]
#          scores = [first_attempt_score, second_attempt_score, third_attempt_score, fourth_attempt_score]
#
#          minIndex = scores.index(min(scores))
#
#          winning_params = params[minIndex]
#
#        else:
#
#          winning_params = first_attempt_params
        ...
        first_attempt_score = ll.Value(first_attempt_params)
        all_params.append(first_attempt_params)
        all_scores.append(first_attempt_score)
        
        # Check conditions after first attempt
        phi_est = np.arctan2(first_attempt_params[1], first_attempt_params[0]) % (2*np.pi)
        theta_est = 1.0 * np.arccos(first_attempt_params[2]) % (np.pi/2)
        conditionA = abs(theta_est - np.pi/2) <= 9999999999999999999999#2*np.pi/180
        conditionB = any(abs(phi_est - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
        
        # Initialize best parameters and score with first attempt 
        # (needed to avoid UnboundLocalError in subsequent comparisons)
        best_score = first_attempt_score
        best_params = first_attempt_params
        
        # Check if first attempt is valid
        found_valid_solution = not (conditionA or conditionB)
        
        # If first attempt provided a valid solution, no need for further attempts
        if not found_valid_solution:
            # Second attempt
            print('    Attempt 2...')
            secondStartValues = pinit.copy()
            secondStartValues[0] = np.sin(5*np.pi/180)*np.cos(phi_est)
            secondStartValues[1] = np.sin(5*np.pi/180)*np.sin(phi_est)
            secondStartValues[2] = np.cos(5*np.pi/180)
            res = minimize(ll.Value, secondStartValues, method='Powell', bounds=bounds, options=options)
            second_attempt_params = res.x
            second_attempt_score = ll.Value(second_attempt_params)
            
            # Check conditions after second attempt
            phi_est2 = np.arctan2(second_attempt_params[1], second_attempt_params[0]) % (2*np.pi)
            theta_est2 = 1.0 * np.arccos(second_attempt_params[2]) % (np.pi/2)
            conditionA_2 = abs(theta_est2 - np.pi/2) <= 999999999999999#2*np.pi/180
            conditionB_2 = any(abs(phi_est2 - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
            
            # Check if second attempt is valid and always update best score if better
            second_attempt_valid = not (conditionA_2 or conditionB_2)
            if second_attempt_score < best_score:
                best_params = second_attempt_params
                best_score = second_attempt_score
            
            # Update found_valid_solution flag
            if second_attempt_valid:
                found_valid_solution = True
        
        # Third attempt only if no valid solution found yet
        if not found_valid_solution:
            print('            3...')
            thirdStartValues = pinit.copy()
            thirdStartValues[0] = np.sin(theta_est)*np.cos(45*np.pi/180)
            thirdStartValues[1] = np.sin(theta_est)*np.sin(45*np.pi/180)
            thirdStartValues[2] = np.cos(theta_est)
            res = minimize(ll.Value, thirdStartValues, method='Powell', bounds=bounds, options=options)
            third_attempt_params = res.x
            third_attempt_score = ll.Value(third_attempt_params)
            
            # Check conditions after third attempt
            phi_est3 = np.arctan2(third_attempt_params[1], third_attempt_params[0]) % (2*np.pi)
            theta_est3 = 1.0 * np.arccos(third_attempt_params[2]) % (np.pi/2)
            conditionA_3 = abs(theta_est3 - np.pi/2) <= 999999999999#2*np.pi/180
            conditionB_3 = any(abs(phi_est3 - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
            
            # Check if third attempt is valid
            third_attempt_valid = not (conditionA_3 or conditionB_3)
            # Store third attempt regardless of validity
            all_params.append(third_attempt_params)
            all_scores.append(third_attempt_score)
            
            # Update found_valid_solution flag
            if third_attempt_valid:
                found_valid_solution = True
        
        # Fourth attempt only if no valid solution found yet
        if not found_valid_solution:
            print('            4...')
            fourthStartValues = pinit.copy()
            fourthStartValues[0] = np.sin(5*np.pi/180)*np.cos(45*np.pi/180)
            fourthStartValues[1] = np.sin(5*np.pi/180)*np.sin(45*np.pi/180)
            fourthStartValues[2] = np.cos(5*np.pi/180)
            res = minimize(ll.Value, fourthStartValues, method='Powell', bounds=bounds, options=options)
            fourth_attempt_params = res.x
            fourth_attempt_score = ll.Value(fourth_attempt_params)
            
            # Check conditions after fourth attempt
            phi_est4 = np.arctan2(fourth_attempt_params[1], fourth_attempt_params[0]) % (2*np.pi)
            theta_est4 = 1.0 * np.arccos(fourth_attempt_params[2]) % (np.pi/2)
            conditionA_4 = abs(theta_est4 - np.pi/2) <= 2*np.pi/180
            conditionB_4 = any(abs(phi_est4 - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
            
            # Check if fourth attempt is valid
            fourth_attempt_valid = not (conditionA_4 or conditionB_4)
            # Store fourth attempt regardless of validity
            all_params.append(fourth_attempt_params)
            all_scores.append(fourth_attempt_score)
            
            # Update found_valid_solution flag
            if fourth_attempt_valid:
                found_valid_solution = True
        
        # Fifth attempt only if no valid solution found yet
        if not found_valid_solution:
            print('            5...')
            fifthStartValues = pinit.copy()
            fifthStartValues[0] = np.sin(5*np.pi/180)*np.cos(135*np.pi/180)
            fifthStartValues[1] = np.sin(5*np.pi/180)*np.sin(135*np.pi/180)
            fifthStartValues[2] = np.cos(5*np.pi/180)
            res = minimize(ll.Value, fifthStartValues, method='Powell', bounds=bounds, options=options)
            fifth_attempt_params = res.x
            fifth_attempt_score = ll.Value(fifth_attempt_params)
        
            # Check conditions after fifth attempt
            phi_est5 = np.arctan2(fifth_attempt_params[1], fifth_attempt_params[0]) % (2*np.pi)
            theta_est5 = 1.0 * np.arccos(fifth_attempt_params[2]) % (np.pi/2)
            conditionA_5 = abs(theta_est5 - np.pi/2) <= 2*np.pi/180
            conditionB_5 = any(abs(phi_est5 - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
        
            # Check if fifth attempt is valid
            fifth_attempt_valid = not (conditionA_5 or conditionB_5)
            # Store fifth attempt regardless of validity
            all_params.append(fifth_attempt_params)
            all_scores.append(fifth_attempt_score)
            
            # Update found_valid_solution flag
            if fifth_attempt_valid:
                found_valid_solution = True
        
        # Sixth attempt only if no valid solution found yet
        if not found_valid_solution:
            print('            6...')
            sixthStartValues = pinit.copy()
            sixthStartValues[0] = np.sin(5*np.pi/180)*np.cos(225*np.pi/180)
            sixthStartValues[1] = np.sin(5*np.pi/180)*np.sin(225*np.pi/180)
            sixthStartValues[2] = np.cos(5*np.pi/180)
            res = minimize(ll.Value, sixthStartValues, method='Powell', bounds=bounds, options=options)
            sixth_attempt_params = res.x
            sixth_attempt_score = ll.Value(sixth_attempt_params)
        
            # Check conditions after sixth attempt
            phi_est6 = np.arctan2(sixth_attempt_params[1], sixth_attempt_params[0]) % (2*np.pi)
            theta_est6 = 1.0 * np.arccos(sixth_attempt_params[2]) % (np.pi/2)
            conditionA_6 = abs(theta_est6 - np.pi/2) <= 2*np.pi/180
            conditionB_6 = any(abs(phi_est6 - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
        
            # Check if sixth attempt is valid
            sixth_attempt_valid = not (conditionA_6 or conditionB_6)
            # Store sixth attempt regardless of validity
            all_params.append(sixth_attempt_params)
            all_scores.append(sixth_attempt_score)
            
            # Update found_valid_solution flag
            if sixth_attempt_valid:
                found_valid_solution = True
        
        # Seventh attempt only if no valid solution found yet
        if not found_valid_solution:
            print('            7...')
            seventhStartValues = pinit.copy()
            seventhStartValues[0] = np.sin(5*np.pi/180)*np.cos(315*np.pi/180)
            seventhStartValues[1] = np.sin(5*np.pi/180)*np.sin(315*np.pi/180)
            seventhStartValues[2] = np.cos(5*np.pi/180)
            res = minimize(ll.Value, seventhStartValues, method='Powell', bounds=bounds, options=options)
            seventh_attempt_params = res.x
            seventh_attempt_score = ll.Value(seventh_attempt_params)
        
            # Check conditions after seventh attempt
            phi_est7 = np.arctan2(seventh_attempt_params[1], seventh_attempt_params[0]) % (2*np.pi)
            theta_est7 = 1.0 * np.arccos(seventh_attempt_params[2]) % (np.pi/2)
            conditionA_7 = abs(theta_est7 - np.pi/2) <= 2*np.pi/180
            conditionB_7 = any(abs(phi_est7 - angle) <= 15*np.pi/180 for angle in np.array([0, 90, 180, 270, 360]) * np.pi/180)
        
            # Check if seventh attempt is valid
            seventh_attempt_valid = not (conditionA_7 or conditionB_7)
            # Store seventh attempt regardless of validity
            all_params.append(seventh_attempt_params)
            all_scores.append(seventh_attempt_score)
        
        # Find the best solution from all attempts
        min_score_index = all_scores.index(min(all_scores))
        best_params = all_params[min_score_index]
        best_score = all_scores[min_score_index]
