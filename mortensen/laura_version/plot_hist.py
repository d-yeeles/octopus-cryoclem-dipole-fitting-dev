...
file_paths = [
    '../../hinterer/simulate-multiple/output/1spot_finaltest/background0/fitting_results_gaussian_lowN.py',
    '../../hinterer/simulate-multiple/output/1spot_finaltest/background0/fitting_results_hinterer_lowN.py',
...
for inclination in [68]:#0, 23, 45, 68, 90]:
    ...
    for i, dataset in enumerate(datasets):
        ...
        sns.histplot(data=data, ax=axs[i, 2], kde=True, fill=True, bins=bins)
        ...
        sns.histplot(data=data, ax=axs[i, 3], kde=True, fill=True, bins=bins)
    ...
    plt.tight_layout()
    output_filename = f"hist_inc{round(inclination)}.png"
