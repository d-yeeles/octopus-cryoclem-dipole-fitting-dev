...
def run_octave_optimization(datamatrix, initvals, psf_generator=None):
    """
    Run Octave optimization using fmincon directly, without the unit vector constraint.
    ...
    datamatrix : array_like
        The image data
    initvals : array_like
        Initial parameter values [x[0], x[1], x[2], mux_nm, muy_nm, n_photons]
    psf_generator : callable, optional
        Function to generate PSF (not used in this version)
    ...
    """
    # Create a temporary directory for Octave files
    temp_dir = tempfile.mkdtemp()
    
    try:
        # No normalization needed since we're removing the constraint
        normalized_initvals = initvals.copy()
        
        # Create the function files
        # 1. PSF generator (simple placeholder)
        with open(os.path.join(temp_dir, 'generate_psf.m'), 'w') as f:
            f.write("""
            function psf = generate_psf(phi, theta, mux_nm, muy_nm, n_photons, datamatrix)
                [rows, cols] = size(datamatrix);
                [y, x] = meshgrid(1:rows, 1:cols);
                psf = n_photons * exp(-((x - mux_nm).^2 + (y - muy_nm).^2) / 100);
                psf = reshape(psf, size(datamatrix));
            end
            """)
        ...
        with open(os.path.join(temp_dir, 'lnpdf.m'), 'w') as f:
            f.write("""
            function currentlnpdf = lnpdf(z, params)
                phi = mod(atan2(params(2), params(1)), 2*pi);
                theta = mod(acos(params(3)), pi/2);
                mux_nm = params(4);
                muy_nm = params(5);
                n_photons = params(6);
                
                currentPSF = generate_psf(phi, theta, mux_nm, muy_nm, n_photons, z);
                currentlnpdf = sum(sum(z .* log(currentPSF) - currentPSF - gammaln(z+1)));
            end
            """)
        
        # 3. Cost function
        with open(os.path.join(temp_dir, 'costFunction.m'), 'w') as f:
            f.write("""
            function cost = costFunction(x, datamatrix)
                cost = -lnpdf(datamatrix, x);
            end
            """)
        
        # 4. Main optimization function (removed nonlcon)
        with open(os.path.join(temp_dir, 'run_optimization.m'), 'w') as f:
            f.write("""
            function result = run_optimization(datamatrix, initvals)
                pkg load optim
                
                % Define bounds
                lower_bounds = [-1, -1, -1, -459, -459, 1];  % Changed lower bound for x[2] from 0 to -1
                upper_bounds = [1, 1, 1, 459, 459, 1e10];
                
                % Use initial values as provided
                pinit = initvals;
                
                % Create cost function wrapper
                costFun = @(x) costFunction(x, datamatrix);
                
                % Set optimization options
                options = optimset('Display', 'off', 'TolFun', 1e-6, 'TolX', 1e-6);
                
                % Run optimization without constraint
                [first_attempt, first_cost] = fmincon(costFun, pinit, [], [], [], [], lower_bounds, upper_bounds, [], options);
                
                % Extract angles directly without normalization
                phi_est = atan2(first_attempt(2), first_attempt(1));
                theta_est = acos(first_attempt(3));
                phi_est = mod(phi_est, 2*pi);
                theta_est = mod(theta_est, pi/2);
                
                % No additional angle checks or attempts needed
                winning_params = first_attempt;
                
                % Calculate angles for output
                phi_conv = mod(atan2(winning_params(2), winning_params(1)), 2*pi);
                theta_conv = mod(acos(winning_params(3)), pi/2);
                
                % Return result
                result = [phi_conv, theta_conv, winning_params(4), winning_params(5), winning_params(6)];
            end
            """)
        ...
        results = oc.run_optimization(datamatrix, normalized_initvals)
...
class MLEwT:
    ...
    def Estimate(self, datamatrix):
        ...
        # Run the Octave optimization
        results = run_octave_optimization(datamatrix, self.initvals, self.psf_generator)
