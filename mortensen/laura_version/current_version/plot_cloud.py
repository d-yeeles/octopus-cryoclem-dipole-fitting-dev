...
datasets = [data_mortensen]
model_names = ['mortensen']
module_names = ['mortensen']
file_paths = [
    '../2025-03-19-version/fitting_results_mortensen.py',
]
...
for i, inclination in enumerate([0, 23, 45, 68, 90]):
        ...
        scatter = axs[0, i].scatter(filtered_para, filtered_perp, s=2, c=filtered_az_err, cmap='rainbow')#, vmin=0, vmax=90)
...
#            axs[i, j].text(x, y, f'{az:.1f}', fontsize=6, color='black', ha='left', va='bottom', alpha=0.7)
        ...
        axs[0, i].set_xlabel('Parallel error (Δ$\parallel$), nm')
        axs[0, i].set_ylabel('Perpendicular error (Δ$\perp$), nm')
        axs[0, i].set_title(
            f"{model_names[0]} \n θ = {inclination}°"
        )
        
        # Add colorbar
        cbar = fig.colorbar(scatter, ax=axs[0, i], pad=0.01)
        ...
        axs[0, i].set_xlim(-max_val*1.1, max_val*1.1)
        axs[0, i].set_ylim(-max_val*1.1, max_val*1.1)
        
        # Force aspect ratio to be 1:1
        axs[0, i].set_aspect('equal')
        axs[0, i].grid(True, linestyle='--', alpha=0.7)
        axs[0, i].axhline(y=0, color='gray', linestyle='-', alpha=0.5)
        axs[0, i].axvline(x=0, color='gray', linestyle='-', alpha=0.5)
        ...
        # Single scatter plot with para_err on x-axis and perp_err on y-axis
        scatter = axs[1, i].scatter(filtered_para, filtered_perp, s=2, c=filtered_az_err, cmap='rainbow')#, vmin=0, vmax=90)
...
#            axs[i, j].text(x, y, f'{az:.1f}', fontsize=6, color='black', ha='left', va='bottom', alpha=0.7)
        ...
        # Add axis labels and title
        axs[1, i].set_xlabel('Parallel error (Δ$\parallel$), nm')
        axs[1, i].set_ylabel('Perpendicular error (Δ$\perp$), nm')
        axs[1, i].set_title(
            f"{model_names[0]} \n θ = {inclination}°"
        )
        
        # Add colorbar
        cbar = fig.colorbar(scatter, ax=axs[1, i], pad=0.01)
        cbar.set_label('Phi error')
        
        # Add a reference circle showing the IQR*5 boundary
        max_val = max(abs(filtered_para.max()), abs(filtered_para.min()), 
                      abs(filtered_perp.max()), abs(filtered_perp.min()))
        axs[1, i].set_xlim(-max_val*1.1, max_val*1.1)
        axs[1, i].set_ylim(-max_val*1.1, max_val*1.1)
        
        # Force aspect ratio to be 1:1
        axs[1, i].set_aspect('equal')
        axs[1, i].grid(True, linestyle='--', alpha=0.7)
        axs[1, i].axhline(y=0, color='gray', linestyle='-', alpha=0.5)
        axs[1, i].axvline(x=0, color='gray', linestyle='-', alpha=0.5)
