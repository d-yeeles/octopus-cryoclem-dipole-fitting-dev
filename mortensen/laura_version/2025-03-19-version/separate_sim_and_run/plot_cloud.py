...
file_paths = [
    '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/laura_data/25-03-19/lowN/sparse_theta/fitting_results_mortensen.py',
    'fitting_results_mortensen_allinone2.py',
...
for inclination in [68]:#[0, 23, 45, 68, 90]:
    ...
    for i, dataset in enumerate(datasets):
        ...
        N = max(1, len(filtered_para) // 20)  # Adjust this number to control density of labels
        for j in range(0, len(filtered_para), N):
            axs[i, 1].annotate(
                f"{filtered_az_err[j]:.1f}°",  # Format to 1 decimal place with degree symbol
                (filtered_para[j], filtered_perp[j]),
                fontsize=6,  # Small font size to avoid overcrowding
