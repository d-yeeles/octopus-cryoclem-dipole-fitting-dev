...
% Input params
frames_dir = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_vary_az_reduced/sims_highN/inc68/';

% Output file
fitting_results_path = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_vary_az_reduced/fitting_results_blah.py';
...
for frame_index = 1:length(frame_paths)%randperm(length(frame_paths), 40)
    ...
    patch_width_px = 19;%roundToOdd(patch_width_nm/pixel_size_nm); % Need to generalise this to be appropriate for any pixel size
    patch_width_nm = patch_width_px*pixel_size_nm;%1000; % size of patch around blob to consider

    % And convert to image coords for later
    positionX_px_array = [];
    positionY_px_array = [];
    ...
    outlined_image = (psf_image - min(psf_image(:))) / (max(psf_image(:)) - min(psf_image(:)));
    ...
    for blob_index = 1:length(x_array)
        ...
        % centroid_x_px = positionX_px_array(blob_index);
        % centroid_y_px = positionY_px_array(blob_index);
        % % Compute patch boundaries, ensuring symmetry and correct centering
        % patch_start_x_nm = centroid_x_nm - patch_width_nm/2;
        % patch_start_y_nm = centroid_y_nm - patch_width_nm/2;
        % patch_end_x_nm = centroid_x_nm + patch_width_nm/2;
        % patch_end_y_nm = centroid_y_nm + patch_width_nm/2;
        % % patch_start_x_px = floor(centroid_x_px - (patch_width_px + 1)/2);
        % % patch_start_y_px = floor(centroid_y_px - (patch_width_px + 1)/2);
        % % patch_end_x_px = ceil(centroid_x_px + (patch_width_px + 1)/2) + 1;
        % % patch_end_y_px = ceil(centroid_y_px + (patch_width_px + 1)/2) + 1;
        % patch_start_x_px = ceil(nm_to_px(patch_start_x_nm, pixel_size_nm, image_size_px, 'x'));
        % patch_start_y_px = ceil(nm_to_px(patch_start_y_nm, pixel_size_nm, image_size_px, 'x'));
        % patch_end_x_px = floor(nm_to_px(patch_end_x_nm, pixel_size_nm, image_size_px, 'x'));
        % patch_end_y_px = floor(nm_to_px(patch_end_y_nm, pixel_size_nm, image_size_px, 'x'));
        % 
        % disp(patch_start_x_px)
        % disp(patch_end_x_px)
        % 
        % % Ensure patch boundaries stay within image limits
        % [image_height, image_width] = size(psf_image);
        % 
        % % % Adjust boundaries if the patch exceeds the image bounds
        % % if patch_start_x_px < 1
        % %     patch_start_x_px = 1;
        % %     patch_end_x_px = patch_start_x_px + patch_width_px - 1;  % Adjust the end to compensate
        % % elseif patch_end_x_px > image_width
        % %     patch_end_x_px = image_width;
        % %     patch_start_x_px = patch_end_x_px - patch_width_px + 1;  % Adjust the start to compensate
        % % end
        % % 
        % % if patch_start_y_px < 1
        % %     patch_start_y_px = 1;
        % %     patch_end_y_px = patch_start_y_px + patch_width_px - 1;  % Adjust the end to compensate
        % % elseif patch_end_y_px > image_width
        % %     patch_end_y_px = image_width;
        % %     patch_start_y_px = patch_end_y_px - patch_width_px + 1;  % Adjust the start to compensate
        % % end
        % 
        % % Extract the patch from the original image
        % patch_image = psf_image(patch_start_y_px:patch_end_y_px, patch_start_x_px:patch_end_x_px);        
        % 
        % % % Clip values just if want to display patch
        % % display_image = (patch_image - min(patch_image(:))) / (max(patch_image(:)) - min(patch_image(:)));
        % % figure;
        % % imshow(display_image)
        % 
        % % Generate throwaway PSF object, then replace the image with extracted patch
        % % Need to set image size param to be patch size now
        % par.nPixels = length(patch_image);
        % par.position = Length([0 0 0], 'nm');
        % psfInit = PSF(par);
        % % Store the extracted patch in PSF object
        % psfInit.image = patch_image;
        ...
        blob_time = toc;
        fprintf('  Time for this blob: %.2f seconds\n', blob_time);
        ...
        % % If patching, use this
        % % So far position estimates are on the patch. Need to convert that
        % % back to a position on the original image
        % 
        % fprintf('   Tru: %.4f\n', positionX_nm_array(blob_index));
        % fprintf('   Est: %.4f\n', positionX_nm_estimate);
        % 
        % patch_centre_x_px = patch_start_x_px + (patch_end_x_px - patch_start_x_px)/2;
        % patch_centre_x_nm = px_to_nm(patch_centre_x_px, pixel_size_nm, image_size_px, 'x');
        % positionX_nm_estimate = positionX_nm_estimate + patch_centre_x_nm;
        % fprintf('   Est: %.4f\n', positionX_nm_estimate);
        % 
        % patch_centre_y_px = patch_start_y_px + (patch_end_y_px - patch_start_y_px)/2;
        % patch_centre_y_nm = px_to_nm(patch_centre_y_px, pixel_size_nm, image_size_px, 'y');
        % positionY_nm_estimate = positionY_nm_estimate + patch_centre_y_nm;
        ...
        % If masking or doing nothing
        objective_function_true = -sum(psf_image .* log(ground_truth_PSF) - ground_truth_PSF - log(gamma(psf_image + 1)), 'all');
        % % If patching
        % objective_function_true = -sum(patch_image .* log(ground_truth_PSF) - ground_truth_PSF - log(gamma(patch_image + 1)), 'all');
