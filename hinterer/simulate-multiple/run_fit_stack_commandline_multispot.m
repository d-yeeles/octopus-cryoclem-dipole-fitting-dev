function run_fit_stack_commandline_multispot(varargin)
    if nargin < 5
        error('Usage: matlab -nodisplay -r "run_fit_stack_commandline_multispot(''stack_path'', ''params_path'', ''results_path'', ''model'', ''patch_width_nm'', ''starting_frame_index'', ''ending_frame_index'')"');
    end
    ...
    fit_stack_commandline_multispot(stack_path, params_path, results_path, model, patch_width_nm, starting_frame_index, ending_frame_index);
    exit;
end
