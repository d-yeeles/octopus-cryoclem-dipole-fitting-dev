function fit_stack_commandline_thunderstorm_posterior(input_image_path, input_thunderstorm_path, output_results_path, model, patch_width_nm, first_frame, last_frame)
    ...
    for frame_index = first_frame:last_frame
        ...
        for blob_index = 1:length(current_frame_x_array)
            ...
            parEst.parameterBounds.theta = [0, pi/2];
            parEst.parameterBounds.phi = [0, 2*pi];
            ...
            parEst.parameterStartValues.theta = (pi/2)*rand();
            parEst.parameterStartValues.phi = (2*pi)*rand();
            ...
            % % Create the fitting object (same as before)
            % fitResult = FitPSF_posterior(psfInit, parEst, model);
            % % Configure nested sampling if desired
            % fitResult = fitResult.setNestedSamplingParams('Nlive', 1000, 'tolerance', 0.05, 'plotPosteriors', false);

            % Use the adaptive two-stage approach (... Nlive_simple, Nlive_complex, tol_simple, tol_complex)
            fitResult = twoStagePSFFitting(psfInit, parEst, model, 500, 800, 0.05, 0.1);

            % % Get point estimates (same format as before)
            % estimates = fitResult.estimatesPositionDefocus.ML;
            % % Get parameter summary with uncertainties
            % summary = fitResult.getParameterSummary();
            % % Access additional results
            % log_evidence = fitResult.logEvidence;
            % uncertainties = fitResult.parameterUncertainties;
            % credible_intervals = fitResult.credibleIntervals;
            ...
            if strcmpi(model, 'gaussian')
                ...
                photons_fit_estimate = fitResult.estimatesPositionDefocus.ML(6);
            ...
            blob_time = toc;
            fprintf('  Time for this dipole: %.2f seconds\n', blob_time);
