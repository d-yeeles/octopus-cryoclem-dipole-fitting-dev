...
model_names = ['no fudge, LS', 'no fudge, no LS', 'fudge, LS', 'fudge, no LS']
module_names = ['no fudge, LS', 'no fudge, no LS', 'fudge, LS', 'fudge, no LS']
file_paths = [
    'fitting_results_hinterer_lowN_inc68_nofudge_LS.py',
    'fitting_results_hinterer_lowN_inc68_nofudge_noLS.py',
    'fitting_results_hinterer_lowN_inc68_fudge_LS.py',
    'fitting_results_hinterer_lowN_inc68_fudge_LS_morepoints.py',
]
...
for inclination in [68]:#0, 23, 45, 68, 90]:
    ...
    for i, dataset in enumerate(datasets):
        ...
        scatter = axs[i].scatter(filtered_para, filtered_perp, s=2, c=filtered_az_err, cmap='rainbow')#, vmin=0, vmax=90)
    ...
    plt.tight_layout()
    output_filename = f"LS_chunk_comparison_inc{round(inclination)}.png"
