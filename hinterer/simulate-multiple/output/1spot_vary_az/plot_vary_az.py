...
subdirectory = "./"
for filename in os.listdir(subdirectory):
    if filename.startswith("fitting_results_hinterer_highN_fmincon."):
...
for inclination in [0, 23, 45, 68, 90]:
    ...
    axs[2, 2].set_xlim(mean1-2*max(std1, std2), mean1+2*max(std1, std2))
    axs[2, 2].set_ylim(mean2-2*max(std1, std2), mean2+2*max(std1, std2))
