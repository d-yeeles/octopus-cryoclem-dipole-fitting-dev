...
for inclination in [0, 23, 45, 68, 90]:
    ...
    # Plot 2D histogram
    axs[0, 1].scatter(para_err_centre, perp_err_centre, color=dyellow, alpha=0.15, s=2)
...
#        )
    ...
    axs[0, 1].set_title(f'Localisation residuals histogram - Hinterer centre only (Inclination: {inclination}°)')
...
#    axs[2, 2].set_title(f'Azimuth residuals in green cluster')
    ...
    plt.tight_layout()
#    if inclination == 68:
    output_filename = f"clusters_inc{round(inclination)}_swarm.png"
