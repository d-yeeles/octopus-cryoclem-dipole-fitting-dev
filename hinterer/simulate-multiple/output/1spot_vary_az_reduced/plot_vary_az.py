...
subdirectory = "./"
for filename in os.listdir(subdirectory):
    if filename.startswith("fitting_results_hinterer_lowN_swarm_reparam."):
...
model_names = ['gaussian', 'hinterer highN', 'hinterer lowN']
...
for inclination in [0, 23, 45, 68, 90]:
    ...
    axs[2, 4].scatter(data_hinterer_ps_fixed_inc["az_tru"], data_hinterer_ps_fixed_inc["obj_err"], s=2)
