...
model_names = ['gaussian 1e6 photons', 'hinterer 1e6 photons', 'gaussian 500 photons', 'hinterer 500 photons']
module_names = ['gaussian_fm', 'gaussian_ps', 'hinterer_fm', 'hinterer_ps']
file_paths = [
    'fitting_results_gaussian_highN_fmincon_reparam_cos.py',
    'fitting_results_hinterer_highN_fmincon_reparam_cos_fudge.py',
    'fitting_results_gaussian_lowN_fmincon_reparam_cos.py',
    'fitting_results_hinterer_lowN_fmincon_reparam_cos_fudge.py',
]
...
#        axs[i, 3].set_ylim(0, 180)
        ...
        std = np.std(data)
        se = std / np.sqrt(np.size(data))
        axs[i, 4].scatter(dataset_inc["az_tru"], data, s=2)
