...
model_names = ['gaussian fmincon', 'gaussian swarm', 'hinterer fmincon', 'hinterer fmincon fudge']
module_names = ['gaussian_fm', 'gaussian_ps', 'hinterer_fm', 'hinterer_ps']
file_paths = [
    '../fitting_results_gaussian_highN_fmincon_reparam_cos.py',
    '../fitting_results_gaussian_highN_swarm_reparam_cos.py',
    '../fitting_results_hinterer_highN_fmincon_reparam_cos.py',
    'fitting_results_hinterer_fmincon_180fudge_G.py'
]
...
for inclination in [0, 23, 45, 68, 90]:
    ...
    for i, dataset in enumerate(datasets):
        ...
        axs[i, 0].scatter(dataset_inc["az_tru"], dataset_inc["para_err"], s=2)
        axs[i, 0].set_xlabel('$\phi$, °')
        axs[i, 0].set_ylabel('Δ$\parallel$, nm')
        axs[i, 0].set_title(model_names[i]+"\nParallel localisation residuals")

        axs[i, 1].scatter(dataset_inc["az_tru"], dataset_inc["perp_err"], s=2)
        axs[i, 1].set_xlabel('$\phi$, °')
        axs[i, 1].set_ylabel('Δ$\perp$, nm')
        axs[i, 1].set_title(model_names[i]+"\nPerpendicular localisation residuals")

        axs[i, 2].scatter(dataset_inc["az_tru"], dataset_inc["inc_est"], s=2)
        axs[i, 2].set_xlabel('$\phi$, °')
        axs[i, 2].set_ylabel('Δθ, °')
        axs[i, 2].set_title(model_names[i]+"\nPolar residuals")

        axs[i, 3].scatter(dataset_inc["az_tru"], dataset_inc["az_est"], s=2)
        axs[i, 3].set_xlabel('$\phi$, °')
        axs[i, 3].set_ylabel('Δ$\phi$, °')
        axs[i, 3].set_title(model_names[i]+"\nAzimuth residuals")

        axs[i, 4].scatter(dataset_inc["az_tru"], abs(dataset_inc["obj_err"]), s=2)
