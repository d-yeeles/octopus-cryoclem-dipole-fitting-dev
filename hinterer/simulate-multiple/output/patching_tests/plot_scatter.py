...
file_paths = [
    './results_9spot_stitched_stack_thunderstorm.py',
]
...
output_dir = './results_9spot_stitched_stack_thunderstorm/'
...
for inclination in inclinations:
    ...
    axs[0].scatter(dataset_inc["az_tru"], data, s=2)
    axs[0].axhline(mean, color=dred, linewidth=1)
    axs[0].fill_between(np.linspace(0, 360), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
    axs[0].set_ylim(-45, 45)
    axs[0].set_ylabel('Δ$\parallel$, nm')
    ...
    std = np.std(data)
    se = std / np.sqrt(np.size(data))
    axs[1].scatter(dataset_inc["az_tru"], data, s=2)
    axs[1].axhline(mean, color=dred, linewidth=1)
    axs[1].fill_between(np.linspace(0, 360), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
    axs[1].set_ylim(-45, 45)
    axs[1].set_xlabel('$\phi$, °')
    axs[1].set_ylabel('Δ$\perp$, nm')
    ...
    std = np.std(data)
    se = std / np.sqrt(np.size(data))
    axs[2].scatter(dataset_inc["az_tru"], data, s=2)
    axs[2].axhline(mean, color=dred, linewidth=1)
    axs[2].fill_between(np.linspace(0, 360), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
    ...
    std = np.std(data)
    se = std / np.sqrt(np.size(data))
    axs[3].scatter(dataset_inc["az_tru"], data, s=2)
    axs[3].axhline(mean, color=dred, linewidth=1)
    axs[3].fill_between(np.linspace(0, 360), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
    axs[3].set_xlabel('$\phi$, °')
    axs[3].set_ylim([-10, 190])
    axs[3].set_ylabel('Δ$\phi$, °')
    ...
    std = np.std(data)
    se = std / np.sqrt(np.size(data))
    axs[4].scatter(dataset_inc["az_tru"], data, s=2)
    axs[4].axhline(mean, color=dred, linewidth=1)
    axs[4].fill_between(np.linspace(0, 360), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
    axs[4].set_xlabel('$\phi$, °')
    axs[4].set_ylim([-220, 1220])
    axs[4].set_ylabel('ΔN, °')
    axs[4].set_title(f"{model_names[i]}\nPhoton count residuals\n" 
                        f"μ = {mean:.4f}, σ = {std:.4f}, μ/SE = {mean/se:.4f}")

    plt.tight_layout()
    output_filename = f"scatter_inc{round(inclination)}.png"
