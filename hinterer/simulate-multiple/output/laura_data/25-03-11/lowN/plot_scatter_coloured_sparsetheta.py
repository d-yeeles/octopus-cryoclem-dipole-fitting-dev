...
for inclination in [0, 23, 45, 68, 90]:
    ...
    for i, dataset in enumerate(datasets):
        ...
        inc_values = dataset_inc["az_err"]  # Get inclination values for coloring
        ...
        filtered_az = dataset_inc["az_tru"][mask]
        filtered_inc = inc_values[mask]  # Filter inclination values too
        
        mean = np.mean(filtered_data)
        std = np.std(filtered_data, ddof=1)
        se = std / np.sqrt(np.size(filtered_data))
        
        # Use rainbow colormap for scatter points based on inc_tru values
        scatter = axs[i, 0].scatter(filtered_az, filtered_data, s=2, c=filtered_inc, cmap='rainbow')
        ...
        # Use rainbow colormap for scatter points based on inc_tru values
        scatter = axs[i, 1].scatter(filtered_az, filtered_data, s=2, c=filtered_inc, cmap='rainbow')
    ...
    plt.tight_layout()
    output_filename = f"scatter_coloured_plots_inc{round(inclination)}.png"
