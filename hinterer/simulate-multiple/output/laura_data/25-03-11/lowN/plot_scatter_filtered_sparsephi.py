...
file_paths = [
    ...
    'sparse_phi/fitting_results_mortensen.py',
]
...
output_dir = './sparse_phi/'

# Azimuth plots

# Generate plots for each fixed inclination
for azimuth in [0, 45, 135, 180]:
   
    fig, axs = plt.subplots(3, 4, figsize=(30, 15))

    for i, dataset in enumerate(datasets):

        dataset_inc = dataset[np.abs(dataset['az_tru'] - azimuth) <= 5]
        ...
        filtered_az = dataset_inc["inc_tru"][(data >= lower_bound) & (data <= upper_bound)]
        ...
        axs[i, 0].fill_between(np.linspace(0, 90), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
#        axs[i, 0].set_ylim(-50, 50)
        axs[i, 0].set_xlabel('θ, °')
        ...
        axs[i, 1].fill_between(np.linspace(0, 90), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
#        axs[i, 1].set_ylim(-50, 50)
        axs[i, 1].set_xlabel('θ, °')
        ...
        axs[i, 2].fill_between(np.linspace(0, 90), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
#        axs[i, 2].set_ylim(0, 90)
        axs[i, 2].set_xlabel('θ, °')
        ...
        axs[i, 3].fill_between(np.linspace(0, 90), mean-abs(std), mean+abs(std), color='red', alpha=0.1)
#        axs[i, 3].set_ylim(0, 180)
        axs[i, 3].set_xlabel('θ, °')
    ...
    plt.tight_layout()
    #plt.show()
    output_filename = f"scatter_filtered_plots_az{round(azimuth)}.png"
