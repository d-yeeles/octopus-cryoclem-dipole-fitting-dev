...
def compute_para_perp(x_errors, y_errors, inclinations, azimuths):
...
#    para_errors = x_errors*np.sin(inclinations)*np.cos(azimuths) + y_errors*np.sin(inclinations)*np.sin(azimuths)
#    perp_errors = np.sqrt((x_errors**2 + y_errors**2) - (para_errors)**2)
...
model_names = ['gaussian 2000 photons 0 background', 'hinterer 2000 photons 0 background', 'mortensen 2000 photons 0 background']
module_names = ['gaussian', 'hinterer', 'mortensen']
file_paths = [
    '../../1spot_finaltest/fitting_results_gaussian_lowN.py',
    '../../1spot_finaltest/fitting_results_hinterer_lowN.py',
    'fitting_results_mortensen_lowN_theta23.py',
]
...
for i, dataset in enumerate(datasets):
    ...
    if os.path.exists(file_path):
        ...
        newdata = pd.DataFrame({
                ...
                "para_err": compute_para_perp(np.array(module.x_tru) - np.array(module.x_est), np.array(module.y_tru) - np.array(module.y_est), module.inc_tru, module.az_tru)[0],
                "perp_err": compute_para_perp(np.array(module.x_tru) - np.array(module.x_est), np.array(module.y_tru) - np.array(module.y_est), module.inc_tru, module.az_tru)[1],
...
for inclination in [23]:#[0, 23, 45, 68, 90]:
    ...
    for i, dataset in enumerate(datasets):
        ...
        axs[i, 0].scatter(dataset_inc["az_tru"], data, s=2)
...
#        axs[i, 0].set_ylim(-50, 50)
        ...
        std = np.std(data)
        se = std / np.sqrt(np.size(data))
        axs[i, 1].scatter(dataset_inc["az_tru"], data, s=2)
...
#        axs[i, 1].set_ylim(-50, 50)
        ...
        std = np.std(data)
        se = std / np.sqrt(np.size(data))
        axs[i, 2].scatter(dataset_inc["az_tru"], data, s=2)
...
#        axs[i, 2].set_ylim(0, 90)
        ...
        std = np.std(data)
        se = std / np.sqrt(np.size(data))
        axs[i, 3].scatter(dataset_inc["az_tru"], data, s=2)
...
#        axs[i, 3].set_ylim(0, 180)
    ...
    plt.tight_layout()
    #plt.show()
    output_filename = f"scatter_plots_inc{round(inclination)}.png"
