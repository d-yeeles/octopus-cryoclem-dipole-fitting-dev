...
for frame_index = 1:length(frame_paths)%randperm(length(frame_paths), 40)
    ...
    for blob_index = 1:length(x_array)
        ...
        centroid_x_nm = positionX_nm_array(blob_index) + (rand - 0.5) * 100; % this is simulating some other blob-finding result
        centroid_y_nm = positionY_nm_array(blob_index) + (rand - 0.5) * 100;
