% function fit_stack_commandline_multispot(stack_path, params_path, results_path, model, patch_width_nm, starting_frame_index, ending_frame_index)
...
%         image_size_nm = frame_params{1, 9};  % Column 7 is image_size_nm
%         image_size_px = frame_params{1, 10};  % Column 8 is image_size_px
% 
%         % Set up parameters for PSF modeling
%         par = struct();
%         par.wavelength = Length(frame_params{1, 12}, 'nm');  % Column 9 is wavelength
%         par.pixelSize = Length(pixel_size_nm, 'nm');
%         par.objectiveNA = frame_params{1, 13};  % Column 10 is objectiveNA
%         par.objectiveFocalLength = Length(frame_params{1, 14}, 'mu');  % Column 11 is objectiveFocalLength
% 
%         % For refractive indices, combine the three columns
%         if length(frame_params{1, 15}) == 1
%             par.refractiveIndices = frame_params{1, 15};
%         else
%             par.refractiveIndices = str2num(frame_params{1, 15}{1});
%         end
% 
% 
%         par.nDiscretizationBFP = frame_params{1, 16};
%         par.backgroundNoise = frame_params{1, 17};
%         par.nPhotons = frame_params{1, 18};
%         par.pixelSensitivityMask = PixelSensitivity.uniform(9);
% 
%         % Get position and angle values directly
%         if length(frame_params{1, 19}) == 1
%             positionX_nm_array = frame_params{1, 19};
%             positionY_nm_array = frame_params{1, 20};
%             angleInclination_array = frame_params{1, 21};
%             angleAzimuth_array = frame_params{1, 22};
%         else
%             positionX_nm_array = str2num(frame_params{1, 19}{1});
%             positionY_nm_array = str2num(frame_params{1, 20}{1});
%             angleInclination_array = str2num(frame_params{1, 21}{1});
%             angleAzimuth_array = str2num(frame_params{1, 22}{1});
%         end
% 
% 
%         % Double check the lengths
%         disp(['Actual number of spots: ', num2str(length(positionX_nm_array))]);
% 
...
function fit_stack_commandline_multispot(stack_path, params_path, results_path, model, patch_width_nm, starting_frame_index, ending_frame_index)
    ...
    number_of_spots = params_table.number_of_spots(1);
    pixel_size_nm = params_table.pixel_size_nm(1);  % Column 6 is pixel_size_nm
    % image_size_nm = params_table.image_size_nm(1);  % Column 7 is image_size_nm
    % image_size_px = params_table.image_size_px(1);  % Column 8 is image_size_px

    % Set up parameters for PSF modeling
    par = struct();
    par.wavelength = Length(params_table.wavelength(1), 'nm');  % Column 9 is wavelength
    par.pixelSize = Length(pixel_size_nm, 'nm');
    par.objectiveNA = params_table.objectiveNA(1);  % Column 10 is objectiveNA
    par.objectiveFocalLength = Length(params_table.objectiveFocalLength(1), 'mu');  % Column 11 is objectiveFocalLength
    ...
    par.nPhotons = params_table.nPhotons(1);
    par.pixelSensitivityMask = PixelSensitivity.uniform(9);
    ...
    for frame_index = starting_frame_index:ending_frame_index
        ...
        for blob_index = 1:number_of_spots
            ...
            row_indices = find(params_table.frame_index == frame_index & params_table.dipole_index == blob_index);
            
            % Read in  position and angle values for this dipole
            positionX_nm_true = params_table.dipole_posX_nm(row_indices);
            positionY_nm_true = params_table.dipole_posY_nm(row_indices);
            angleInclination_true = params_table.angleInclination(row_indices);
            angleAzimuth_true = params_table.angleAzimuth(row_indices);
