...
% Input params
frames_dir = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_finaltest/sims_highN/';

% Output file
fitting_results_path = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_finaltest/fitting_results_hinterer_highN.py';
...
% Output file
fitting_results_path = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_finaltest/fitting_results_gaussian_highN.py';
...
for frame_index = 1:length(frame_paths)%randperm(length(frame_paths), 40)
    ...
    for blob_index = 1:length(x_array)
        ...
        parEst.parameterStartValues.cosAzimuth = 2*(rand()-0.5);
        parEst.parameterStartValues.sinAzimuth = 2*(rand()-0.5);
        fitResult = FitPSF_ML_reparam_gaussian(psfInit, parEst);
        angleInclination_estimate = 0.5*acos(fitResult.estimatesPositionDefocus.ML(4));
        angleAzimuth_estimate = atan2(fitResult.estimatesPositionDefocus.ML(6), fitResult.estimatesPositionDefocus.ML(5));
        positionX_nm_estimate = fitResult.estimatesPositionDefocus.ML(1);
        positionY_nm_estimate = fitResult.estimatesPositionDefocus.ML(2);
        ...
        % Now we need the value of the objective function at ground truth
        ground_truth_params = [positionX_nm_array(blob_index), positionY_nm_array(blob_index), 0, angleInclination_array(blob_index), angleAzimuth_array(blob_index)];
        ground_truth_PSF = createFitPSF_gaussian(psfInit, ground_truth_params); 
        objective_function_true = -sum(psf_image .* log(ground_truth_PSF) - ground_truth_PSF - log(gamma(psf_image + 1)), 'all');
...
% Input params
frames_dir = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_finaltest/sims_lowN/';

% Output file
fitting_results_path = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_finaltest/fitting_results_hinterer_lowN.py';
...
% Output file
fitting_results_path = '/home/tfq96423/Documents/cryoCLEM/dipole-issue/fixed-dipole-issue/hinterer/simulate-multiple/output/1spot_finaltest/fitting_results_gaussian_lowN.py';
