classdef FitPSF_PS_givenazimuth
    ...
    properties
        ...
        parameterBounds = struct( ...
            ...
            'inclination', [-0.0174533, (pi/2)+0.0174533]);          % dave jan 2025 - adding angle optimiser
    ...
    methods
        function obj = FitPSF_PS_givenazimuth(psf, par)
            if nargin > 1
                obj = setInputParameters('FitPSF_PS_givenazimuth', obj, par);
            end
            if nargin > 0
                obj.psf = psf;
                obj.image = psf.image;
        ...
        function estimatesPositionDefocus = fitting(obj)
            parPsfEstimate = FitPSF_PS_givenazimuth.readParametersEstimate(obj.psf);
            parPsfEstimate.dipole = Dipole(0, obj.angleAzimuthEstimate);
        ...
        function estimatesPositionDefocusPS = fitParticleSwarmPSF(obj, image, psfEstimate)
            ...
            lowerBounds = [xBounds(1), yBounds(1), defocusBounds(1), inclinationBounds(1)];
            upperBounds = [xBounds(2), yBounds(2), defocusBounds(2), inclinationBounds(2)];

            % Set options for the Particle Swarm Optimization

            % Create a uniform grid for initial swarm positions
            % numParticlesPerDim = 3;
            [xGrid, yGrid, defocusGrid, inclinationGrid] = ndgrid(...
            ...
            initialSwarm = [xGrid(:), yGrid(:), defocusGrid(:), inclinationGrid(:)];
            numParticles = 36; % this needs to be the multiplicative sum of each param given in the linspace above
        ...
        function currentFitPSF = createFitPSF(obj, psfEstimate, lateralPositionAndDefocus)
            ...
            psfEstimate.dipole = Dipole(lateralPositionAndDefocus(4), obj.angleAzimuthEstimate); % dave jan 2025 - adding angle optimiser
