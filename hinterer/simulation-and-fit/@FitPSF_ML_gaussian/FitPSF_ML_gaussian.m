classdef FitPSF_ML_gaussian
    ...
    methods
        function obj = FitPSF_ML_gaussian(psf, par)
            if nargin > 1
                obj = setInputParameters('FitPSF_ML_gaussian', obj, par);
            end
            if nargin > 0
                obj.psf = psf;
                obj.image = psf.image;
        ...
        function estimatesPositionDefocus = fitting(obj)
            parPsfEstimate = FitPSF_ML_gaussian.readParametersEstimate(obj.psf);
        ...
        function estimatesPositionDefocusML = fitMaxLikelihoodPSF(obj, image, psfEstimate, startValues)
            ...
            options = optimoptions(@fmincon, ...
                'Display', 'off', ...               % Do not display output
                'StepTolerance', 1e-9, ...          % Stop when the step size is less than 1e-6
                'OptimalityTolerance', 1e-9, ...    % Stop when the gradient is less than 1e-6
                'FunctionTolerance', 1e-9, ...        % Stop if the function value change is less than 1e-6
