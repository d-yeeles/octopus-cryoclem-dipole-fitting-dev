classdef FitPSF_posterior
    ...
    methods
        function obj = FitPSF_posterior(psf, par, model, autoFit)
            ...
            if nargin > 1
                obj = setInputParameters('FitPSF_posterior', obj, par);
            end
        ...
        function [estimatesPositionDefocus, logEvidence, posteriorSamples, parameterUncertainties, credibleIntervals] = fitting(obj, model)
            parPsfEstimate = FitPSF_posterior.readParametersEstimate(obj.psf);
            ...
            estimatesPositionDefocus.ML = FitPSF_posterior.extractPointEstimates(post_samples, obj.nestedSamplingParams.estimationMethod);
            
            % Store full results
            posteriorSamples = post_samples;
            
            % Get parameter uncertainties
            [parameterUncertainties, credibleIntervals] = FitPSF_posterior.getParameterUncertainties(post_samples, 0.95);
        end
        ...
        function [logZ, nest_samples, post_samples] = fitNestedSamplingPSF(obj, image, psfEstimate, model)
            ...
            likelihood = @(data_in, model_func, parnames, parvals) FitPSF_posterior.validateAndCallLikelihood(obj, psfEstimate, data_in, parnames, parvals);
            ...
            if strcmpi(model, 'gaussian')
                ...
                % Define priors for Hinterer/Mortensen model
                xBounds = obj.parameterBounds.x.inNanometer;
                yBounds = obj.parameterBounds.y.inNanometer;
                defocusBounds = obj.parameterBounds.defocus.inNanometer;
                ...
                defocusLower = double(defocusBounds(1));
                defocusUpper = double(defocusBounds(2));
                newangle1Lower = double(newangle1Bounds(1));
                newangle1Upper = double(newangle1Bounds(2));
                newangle2Lower = double(newangle2Bounds(1));
                newangle2Upper = double(newangle2Bounds(2));
                newangle3Lower = double(newangle3Bounds(1));
                newangle3Upper = double(newangle3Bounds(2));
                photonsLower = double(photonsBounds(1));
                photonsUpper = double(photonsBounds(2));
                
                % Additional safety checking for all bounds
                bounds_check = [xLower, xUpper; yLower, yUpper; defocusLower, defocusUpper; ...
                               newangle1Lower, newangle1Upper; newangle2Lower, newangle2Upper; newangle3Lower, newangle3Upper; ...
                               photonsLower, photonsUpper];
                    ...
                    if ~isfinite(bounds_check(i,1)) || ~isfinite(bounds_check(i,2)) || bounds_check(i,1) >= bounds_check(i,2)
                        switch i
                            ...
                            case 3  % defocus bounds
                                defocusLower = -10; defocusUpper = 10;
                            case 4  % theta bounds
                                newangle1Lower = -1; newangle1Upper = 1;
                            case 5  % phi bounds
                                newangle2Lower = -1; newangle2Upper = 1;
                            case 6  % phi bounds
                                newangle3Lower = 0; newangle3Upper = 1;
                ...
                prior = {'x', 'uniform', xLower, xUpper, 'fixed'; ...
                         'y', 'uniform', yLower, yUpper, 'fixed'; ...
                         'defocus', 'uniform', defocusLower, defocusUpper, 'fixed'; ...
                         'newangle1', 'uniform', newangle1Lower, newangle1Upper, 'fixed'; ...
                         'newangle2', 'uniform', newangle2Lower, newangle2Upper, 'fixed'; ...
                         'newangle3', 'uniform', newangle3Lower, newangle3Upper, 'fixed'; ...
                         'photons', 'uniform', photonsLower, photonsUpper, 'fixed'};
            end
            ...
            if obj.nestedSamplingParams.plotPosteriors
                ...
                try
                    if strcmpi(model, 'gaussian')
                        ...
                        % FIX: Corrected to match the number of parameters (6 instead of 7)
                        for i = 1:7
                            posteriors(post_samples, i, {prior{i,1}});
                        end
                        posteriors(post_samples, [1 2], {prior{1,1}, prior{2,1}});
        ...
        function currentFitPSF = createFitPSF(obj, psfEstimate, lateralPositionAndDefocus)
            ...
            if strcmpi(obj.model, 'gaussian')
                ...
                newangle1_raw = lateralPositionAndDefocus(4);
                newangle2_raw = lateralPositionAndDefocus(5);
                newangle3_raw = lateralPositionAndDefocus(6);
                
                % Normalize to unit sphere
                norm_factor = sqrt(newangle1_raw^2 + newangle2_raw^2 + newangle3_raw^2);
                if norm_factor < 1e-10  % Avoid division by zero
                    % Default to vertical dipole if vector is too small
                    newangle1_normalized = 0;
                    newangle2_normalized = 0;
                    newangle3_normalized = 1;
                else
                    newangle1_normalized = newangle1_raw / norm_factor;
                    newangle2_normalized = newangle2_raw / norm_factor;
                    newangle3_normalized = newangle3_raw / norm_factor;
                end
                
                % Convert to spherical coordinates
                inclination = acos(abs(newangle3_normalized));  % abs() ensures [0, π/2]
                azimuth = atan2(newangle2_normalized, newangle1_normalized);
                inclination = mod(inclination, pi/2);
                azimuth = mod(azimuth, 2*pi);
        ...
        function summary = getParameterSummary(obj)
            ...
            if strcmpi(obj.model, 'gaussian')
                ...
                param_names = {'x', 'y', 'defocus', 'newangle1', 'newangle2', 'newangle3', 'photons'};
            end
    ...
    methods (Static)
        ...
        function logL = validateAndCallLikelihood(obj, psfEstimate, data, parnames, parvals)
            try
                ...
                logL = FitPSF_posterior.lnpdfFunction_nested(obj, obj, psfEstimate, data, parnames, parvals);
        ...
        function logL = lnpdfFunction_nested(obj, obj_ref, psfEstimate, data, parnames, parvals)
            ...
            % if debug_count <= 3
            %     fprintf('\n=== DEBUG LIKELIHOOD CALL %d ===\n', debug_count);
            %     fprintf('Number of input arguments: %d\n', nargin);
            %     fprintf('data class: %s, size: %s\n', class(data), mat2str(size(data)));
            %     fprintf('parnames class: %s, length: %d\n', class(parnames), length(parnames));
            %     fprintf('parvals class: %s, length: %d\n', class(parvals), length(parvals));
            % 
            %     if iscell(parnames)
            %         fprintf('parnames contents: ');
            %         for i = 1:length(parnames)
            %             fprintf('%s ', parnames{i});
            %         end
            %         fprintf('\n');
            %     end
            % 
            %     if iscell(parvals)
            %         fprintf('parvals contents: ');
            %         for i = 1:length(parvals)
            %             fprintf('%.3f ', parvals{i});
            %         end
            %         fprintf('\n');
            %     end
            %     fprintf('===============================\n');
            ...
            try
                ...
                if length(params) == 4 && params(4) <= 0  % Gaussian model photons
                    logL = -inf;
                    return;
                elseif length(params) == 7 && params(7) <= 0  % Full model photons
                    logL = -inf;
                    return;
                end
                
                % Call your original likelihood function with the correct object reference
                % Use obj (the FitPSF_posterior object) instead of obj_ref
                ...
                % if debug_count <= 5
                %     fprintf('Likelihood call %d: logL = %.6f (class: %s, size: %s)\n', ...
                %         debug_count, logL, class(logL), mat2str(size(logL)));
                % end
