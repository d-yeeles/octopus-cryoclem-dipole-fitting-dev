classdef PSF_modified
    ...
    methods
        ...
        % dave november 2024 - add to existing image if given
        function obj = createImage(obj)
            ...
            % Initialize psf if image doesn't exist, otherwise use the existing image
            if isempty(obj.image)
                psf = zeros(obj.nPixels, obj.nPixels);
            else
                psf = obj.image;  % Start with the existing image if it exists
            end

            % Accumulate the image values
            ...
            % Adjust excitation, apply shot noise, and add background noise
            psf = adjustExcitation(obj, psf);
            psf = applyShotNoise(obj, psf);
            psf = addBackgroundNoise(obj, psf);

            % Update the image property with the new or accumulated PSF
            obj.image = psf;
        end
...
%     end
% 
%     % Add the imageCombined property to store the combined PSF images
%     properties (Hidden)
%         imageCombined
%         dipoleCombined
%         positionCombined
...
%     methods
%         %% Constructor
%         function obj = PSF_modified(par, psfA)
%             if nargin > 0
%                 if exist('psfA', 'var') && ~isempty(psfA)
%                     % Combine the two PSF objects
%                     obj = obj.combinePSFs(par, psfA);
%                 else
%                     obj = setInputParameters('PSF', obj, par);
%                     obj = obj.createImage(); % Create image if no second PSF is passed
%                 end
%             end
%         end
% 
...
%         end
%         function psf = addBackgroundNoise(obj, psf)
%             psf = psf + obj.backgroundNoise;
%         end
% 
%         %% Combine two PSFs
%         function obj = combinePSFs(obj, par, psfA)
%             % Create the current PSF image
%             obj = setInputParameters('PSF', obj, par);
%             obj = obj.createImage(); % Create image for the current PSF
% 
%             % Check if the other PSF has an image already, if not, create it
%             if isempty(psfA.image)
%                 psfA = psfA.createImage(); % Ensure that psfA has an image created
%             end
% 
%             % Store both PSF images in a cell array for combined object
%             obj.imageCombined = {obj.image, psfA.image};
% 
%             % Combine additional properties like dipole, position if needed
%             obj.dipoleCombined = {obj.dipole, psfA.dipole};  % Store both dipoles
%             obj.positionCombined = {obj.position, psfA.position};  % Store both positions
%         end
% 
%         %% Plot function
%         function plot(obj, setLimitsFullRange)
%             if isfield(obj, 'imageCombined')
%                 % If the combined images exist, plot them together
%                 figure;
%                 subplot(1,2,1);
%                 imagesc(obj.imageCombined{1}, [0 max(obj.imageCombined{1}(:))]);
%                 title('PSF 1');
%                 colorbar;
%                 axis equal; axis tight; axis xy;
% 
%                 subplot(1,2,2);
%                 imagesc(obj.imageCombined{2}, [0 max(obj.imageCombined{2}(:))]);
%                 title('PSF 2');
%                 colorbar;
%                 axis equal; axis tight; axis xy;
%             else
%                 % Plot single PSF object
%                 if nargin == 2 && setLimitsFullRange
%                     imagesc(obj.image(:,:), [0 max(obj.image(:))]);
%                 else
%                     imagesc(flip(obj.image(:,:)));
