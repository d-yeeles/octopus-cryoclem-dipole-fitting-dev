classdef FitPSF_PS_180fudge
    ...
    methods
        function obj = FitPSF_PS_180fudge(psf, par)
            if nargin > 1
                obj = setInputParameters('FitPSF_PS_180fudge', obj, par);
            end
            if nargin > 0
                obj.psf = psf;
                obj.image = psf.image;
        ...
        function estimatesPositionDefocus = fitting(obj)
            parPsfEstimate = FitPSF_PS_180fudge.readParametersEstimate(obj.psf);
        ...
        function estimatesPositionDefocusPS = fitParticleSwarmPSF(obj, image, psfEstimate)
            ...
            firstInitialSwarm = [xGrid(:), yGrid(:), defocusGrid(:), inclinationGrid(:), azimuthGrid(:)];
            numParticles = 72; % this needs to be the multiplicative sum of each param given in the linspace above
            firstInitialSwarm = firstInitialSwarm(1:numParticles, :);
            ...
            options = optimoptions('particleswarm', ...
                ...
                'InitialSwarmMatrix', firstInitialSwarm, ...      % Swarm distribution
                'FunctionTolerance', 1e-9);   % Stopping criteria for function improvement

            % Run Particle Swarm Optimization
            numVariables = numel(lowerBounds);
            firstEstimate = particleswarm(costFunction, numVariables, lowerBounds, upperBounds, options);

            % add in something to save the value of the objetive function
            firstCost = costFunction(firstEstimate);
        
            % Modify azimuth by subtracting/adding π (ensure it stays within bounds)

            % Create a uniform grid for initial swarm positions
            % but this time start them nearby the found solution for
            % azimuth (but flipped)
            % numParticlesPerDim = 3;
            [xGrid, yGrid, defocusGrid, inclinationGrid, azimuthGrid] = ndgrid(...
                linspace(xBounds(1), xBounds(2), 3), ...
                ...
                linspace(defocusBounds(1), defocusBounds(2), 2), ...
                linspace(inclinationBounds(1), inclinationBounds(2), 2), ...
                linspace(firstEstimate(5) - 90*pi/180 - pi , firstEstimate(5) + 90*pi/180 - pi , 2));
            % Reshape grid into an initial swarm matrix
            secondInitialSwarm = [xGrid(:), yGrid(:), defocusGrid(:), inclinationGrid(:), azimuthGrid(:)];
            % secondInitialSwarm(5) = mod(firstEstimate(5) - pi, 2*pi); % Ensure azimuth stays in range
        
            % Run second Particle Swarm Optimization
            options = optimoptions('particleswarm', ...
                'Display', 'off', ...         % Show optimization progress
                'SwarmSize', numParticles, ...          % Number of particles
                'MaxIterations', 1e20, ...      % Maximum number of iterations
                'InitialSwarmMatrix', secondInitialSwarm, ...      % Swarm distribution
                'FunctionTolerance', 1e-9);   % Stopping criteria for function improvement
            
            secondEstimate = particleswarm(costFunction, numVariables, lowerBounds, upperBounds, options);
            secondCost = costFunction(secondEstimate);
        
            % Choose the better estimate
            if secondCost < firstCost
                estimatesPositionDefocusPS = secondEstimate;
                estimatesPositionDefocusPS(end+1) = secondCost;
            else
                estimatesPositionDefocusPS = firstEstimate;
                estimatesPositionDefocusPS(end+1) = firstCost;
            end
        end
        ...
        function currentFitPSF = createFitPSF(obj, psfEstimate, lateralPositionAndDefocus)
            ...
            lateralPositionAndDefocus(5) = mod(lateralPositionAndDefocus(5), pi/2);
