...
datasets = [data_1]
model_names = ['mortensen normalised 1spot\n___ points']
file_paths = [
    './results_1spot_mortensen_normalised.py',
]
...
scatter = axs.scatter(dataset_inc["para_err"], dataset_inc["perp_err"], s=2, c=dataset_inc["az_err"], cmap='rainbow')#, vmin=0, vmax=90)
    
# Add axis labels and title
axs.set_xlabel('Parallel error (Δ$\parallel$), nm')
axs.set_ylabel('Perpendicular error (Δ$\perp$), nm')
axs.set_title(
    f"{model_names[0]}°\n"
)
    
# Add colorbar
cbar = fig.colorbar(scatter, ax=axs, pad=0.01)
cbar.set_label('Phi error')
    
axs.set_xlim(-40, 40)#(-max_val*1.1, max_val*1.1)
axs.set_ylim(-40, 40)#(-max_val*1.1, max_val*1.1)
    
# Force aspect ratio to be 1:1
axs.set_aspect('equal')
axs.grid(True, linestyle='--', alpha=0.7)
axs.set_xticks(np.arange(-40, 40, 10))
axs.set_yticks(np.arange(-40, 40, 10))
