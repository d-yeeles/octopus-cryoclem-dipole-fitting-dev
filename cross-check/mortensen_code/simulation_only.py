    ...
    for run in runs:
      for theta in thetas:
        for phi in phis:
          ...
          # Define output paths
          image_output_path = f'../sims_mortensen/sim_theta{ceil(theta_deg):03}_phi{ceil(phi_deg):03}_run{int(run+1)}.csv'
          params_output_path = f"../sims_mortensen/params_theta{ceil(theta_deg):03}_phi{ceil(phi_deg):03}_run{int(run+1)}.m"
...
#          results_path = "./fitting_results_mortensen_tiff_3.py"
#
