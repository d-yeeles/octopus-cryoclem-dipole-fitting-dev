import numpy as np
from MLEwT_nodeltapix import dipdistr, MLEwT
...
def run_mortensen_fit(image, parameters_path): # change this to accept image + parameters files
...
if __name__ == "__main__":
    ...
    runs = range(1)
    thetas = np.arange(0, np.pi/2 + 0.001, 22.5*np.pi/180)#np.arange(0, np.pi/2 + 0.001, 22.5*np.pi/180)
    phis = np.arange(0, 2*np.pi - 0.001, 45*np.pi/180)
    ...
    for run in runs:
      for theta in thetas:
        for phi in phis:
          ...
          # Define output paths
          image_output_path = f'../sims_hinterer/sim_theta{ceil(theta_deg):03}_phi{ceil(phi_deg):03}_run{int(run+1)}.csv'
          params_output_path = f"../sims_hinterer/params_theta{ceil(theta_deg):03}_phi{ceil(phi_deg):03}_run{int(run+1)}.m"
...
#          file.close()
          ...
          results_path = "../results_mortensen_on_hinterer_test.py"

          # find images and parameter files

          valid_extensions_image = {'.csv'}#{'.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.tif'}
          valid_extensions_settings = {'.m'}

          frame_paths = []
          settings_paths = []

          frames_dir = '../sims_hinterer'
          ...
          # run the fit
          psf_image = np.loadtxt(image_output_path, delimiter=",")
          results, ground_truth = run_mortensen_fit(psf_image, params_output_path)
